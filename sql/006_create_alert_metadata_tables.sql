...
CREATE TABLE IF NOT EXISTS alert_fields (
    ...
    field_type VARCHAR(50) DEFAULT 'string', -- string, number, boolean, ip, port
    description TEXT,
    is_active BOOLEAN DEFAULT true,
    display_order INTEGER DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
(1, 'src_ip', '源IP', 'ip', 14),
(1, 'src_port', '源端口', 'port', 15),
(1, 'dst_ip', '目标IP', 'ip', 16),
(1, 'dst_port', '目标端口', 'port', 17),
...
(1, 'attack_ip', '攻击IP', 'ip', 24),
(1, 'attacked_ip', '被攻击IP', 'ip', 25),
...
(2, 'src_ip', '源IP', 'ip', 14),
(2, 'src_port', '源端口', 'port', 15),
(2, 'dst_ip', '目标IP', 'ip', 16),
(2, 'dst_port', '目标端口', 'port', 17),
...
(2, 'sample_alarm_engine', '告警引擎', 'string', 31),
